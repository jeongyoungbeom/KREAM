...
param = getParameterByName('styleId');
console.log(param);
...
let axiosUrl = '/api/style_detailList/'+sessionId;
if(sessionId==null){
    axiosUrl = '/api/style_detailList/';
}
...
axios.get(axiosUrl, {
    ...
    for(let i in response.data.data){
        ...
        let userText = '<div class="social_user_state">' +
            ...
            '</div>'
        ;
        // sessionId와 게시물의 customerId가 일치하지 않을경우
        if(sessionId != customerId){
            if(followCheck!=true){  // 팔로우 안되어있을 경우
                console.log(followCheck);
                userText += '<button type="button" class="btn_follow" onclick="sendFollow('+customerId+')">팔로우</button>';
            }else{  // 팔로우 되어있을 경우
                console.log(followCheck);
                userText += '<button type="button" class="btn_follow following" onclick="sendFollow('+customerId+')">팔로잉</button>';
            }
        } else if(sessionId==customerId) {
            userText += '<div class="btn_wrapper"><button type="button" class="btn btn_edit" onClick="location.href=\'/social/edit/'+styleId+'\'">수정</button>' +
                '<button type="button" class="btn btn_delete" onclick="styleDelete('+styleId+')">삭제</button></div>'
        }
        userText += '</a></div>';
        ;
        row.append(userText);
        ...
        if(replyCount!=0){      // 댓글 있을 경우
            ...
            for (let i=0; i<replyCount; i++) {
                ...
                social_content += '<span class="dot"> • </span>' +
                    '<span class="like_count"> 공감 <strong id="0'+replyId+'">'+hit+'</strong>개 </span>'
                ;
    ...
    // 특정 영역으로 이동
    console.log('페이지 스크롤 이동하기 ' + param);
    window.setTimeout(function(){
        $('html, body').animate({
            scrollTop: $('#social'+param).offset().top
        }, 200);
    }, 200);
...
function styleDelete(id){
    axios.delete('/api/style_delete/'+id).then(function (){
        location.reload();
...
$(document).on('click', '.btn_follow', function () {
    let index = $(".btn_follow").index(this);
    let btn = document.getElementsByClassName('btn_follow');
    let followingCheck = btn.item(index).classList.contains('following');
    console.log(followingCheck);

    if (sessionId != null) {
        if(followingCheck == true){
            btn.item(index).innerText = "팔로우";
            btn.item(index).classList.remove('following');
        }else {
            btn.item(index).innerText = "팔로잉";
            btn.item(index).classList.add('following');
        }
    } else {
        location.href = '/login';
    }
